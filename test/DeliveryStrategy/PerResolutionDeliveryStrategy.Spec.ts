...
describe('The [PerResolutionDeliveryStrategy]', function() {
    ...
    it('should throw an error if the given resolution context is not valid.', function() {
        assert.throws(() => {
            new PerResolutionDeliveryStrategy().deliver(null, null); 
        }, DeliveryError)
    });

    it('should throw an error if the given dependency metadata is not valid.', function() {
        assert.throws(() => {
            new PerResolutionDeliveryStrategy().deliver(new ResolutionContext(), null); 
        }, DeliveryError)
    });
    ...
    it('should throw an error if the given dependency reference is not valid.', function() {
        assert.throws(() => {
            let dependencyMetadata:DependencyMetadata = new DependencyMetadata();
            new PerResolutionDeliveryStrategy().deliver(new ResolutionContext(), dependencyMetadata); 
        }, DeliveryError)
    });
